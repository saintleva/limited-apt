...
class PrintRunner(RunnerBase):
    ...
    def get_printed_enclosure(self):
        ...
        # sorted sequence
        return (self.display_modes.pkg_str(pkg) for pkg in cache if pkg.candidate is not None and
        ...
        if self.username == "root":
            ...
            for pkg in sorted(changes):
                ...
                if pkg.marked_keep:
                    ...
                    check_fatal()
                if pkg.marked_delete and not pkg.is_auto_removable and not coownership.is_sole_own(concrete_package,
                                                                                                   self.username):
